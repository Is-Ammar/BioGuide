...
/**
 * LiquidEther
 * Full-screen / container-filling fluid-like motion background using THREE.js frame-buffer passes.
 * Interacts with pointer movement and can auto-drive when idle.
 *
 * Basic usage:
 * <div className="absolute inset-0">\n  <LiquidEther />\n</div>
 *
 * Performance tips:
 * - Lower `resolution` (e.g. 0.4) to improve FPS on low-end devices.
 * - Disable viscosity (isViscous=false) for cheaper simulation.
 * - Reduce iterationsPoisson / iterationsViscous for additional savings.
 *
 * Interaction tuning:
 * - pointerSmoothing (0..1) controls low-pass filtering of cursor movement.
 * - inertia enables continued motion after pointer stops; inertiaDecay controls fade-out speed.
 * - maxPointerDelta clamps sudden spikes (useful for erratic touch events / tab switches).
 */
// Lightweight fluid-like velocity field visualization.
// Props allow tuning performance (resolution) and behavior.
export interface LiquidEtherProps {
  ...
  dt?: number;
  BFECC?: boolean;
  resolution?: number; // 0.25 - 1 (fraction of canvas size for simulation)
  isBounce?: boolean;
  colors?: string[];
  ...
  /** 0..1 low-pass filter factor (higher = smoother/slower). */
  pointerSmoothing?: number;
  /** Enable inertial continuation after movement. */
  inertia?: boolean;
  /** Per-second decay factor for inertia velocity. */
  inertiaDecay?: number;
  /** Clamp per-frame normalized delta magnitude. */
...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      dispose() {
        ...
        if (this.useWindowEvents) {
          ...
          c.removeEventListener('mousemove', this._onMouseMove as any);
          c.removeEventListener('touchstart', this._onTouchStart as any);
          c.removeEventListener('touchmove', this._onTouchMove as any);
          c.removeEventListener('mouseenter', this._onMouseEnter as any);
          c.removeEventListener('mouseleave', this._onMouseLeave as any);
          c.removeEventListener('touchend', this._onTouchEnd as any);
        }
      }
      ...
      onDocumentMouseMove(event: MouseEvent) {
        ...
        this.hasUserControl = true;
        // manual hover detection if using window events
        if (this.container && this.useWindowEvents) {
          const rect = this.container.getBoundingClientRect();
          this.isHoverInside = event.clientX >= rect.left && event.clientX <= rect.right && event.clientY >= rect.top && event.clientY <= rect.bottom;
        }
      }
      onDocumentTouchStart(event: TouchEvent) {
        if (event.touches.length === 1) {
          ...
          if (this.container && this.useWindowEvents) {
            const rect = this.container.getBoundingClientRect();
            this.isHoverInside = t.pageX >= rect.left && t.pageX <= rect.right && t.pageY >= rect.top && t.pageY <= rect.bottom;
          }
        }
      }
      ...
      update() {
        ...
        const rawDx = this.coords.x - this.coords_old.x;
        const rawDy = this.coords.y - this.coords_old.y;
        this.coords_old.copy(this.coords);
        let dx = rawDx;
        let dy = rawDy;
        const mag = Math.hypot(dx, dy);
        if (mag > this.maxDelta) {
          const f = this.maxDelta / (mag || 1);
          dx *= f; dy *= f;
        }
        const s = THREE.MathUtils.clamp(this.smoothing, 0, 0.98);
        this.filtered.x += (dx - this.filtered.x) * (1 - s);
        this.filtered.y += (dy - this.filtered.y) * (1 - s);
        if (this.useInertia) {
          const hasMotion = Math.abs(rawDx) > 1e-5 || Math.abs(rawDy) > 1e-5;
          if (hasMotion) {
            this.velocity.x = this.velocity.x * 0.6 + dx * 0.4;
            this.velocity.y = this.velocity.y * 0.6 + dy * 0.4;
          } else {
            const decay = Math.exp(-this.inertiaDecay * (1 / 60));
            this.velocity.multiplyScalar(decay);
            this.filtered.addScaledVector(this.velocity, 0.5);
          }
        }
        this.diff.copy(this.filtered);
    ...
    // GLSL shader strings (trimmed for clarity) ---------------------------
    const face_vert = `
...
`;
    ...
    class ExternalForce extends ShaderPass {
      ...
      constructor(simProps: any) {
        ...
        // custom init with simProps
        this._customInit(simProps);
      }
      // Preserve base signature (no required args) to satisfy TS override
      init(): void { /* noop - use _customInit instead */ }
      private _customInit(simProps: any) {
        super.init();
      ...
      // Accept optional props through rest args to align with base signature
      update(...args: any[]) {
        const props = args[0] || {};
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      constructor(props: any) {
        ...
        if (typeof props.pointerSmoothing === 'number') Mouse.smoothing = props.pointerSmoothing;
        if (typeof props.inertia === 'boolean') Mouse.useInertia = props.inertia;
        if (typeof props.inertiaDecay === 'number') Mouse.inertiaDecay = props.inertiaDecay;
        if (typeof props.maxPointerDelta === 'number') Mouse.maxDelta = props.maxPointerDelta;
        Mouse.onInteract = () => {
